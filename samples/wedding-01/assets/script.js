...
async function submitRsvp(attending) {
  ...
  const guestsVal = parseInt(
    /** @type {HTMLSelectElement} */ (form.elements.namedItem('guests')).value,
    10,
  );
  const messageVal = /** @type {HTMLTextAreaElement} */ (form.elements.namedItem('message')).value.trim();
  ...
  try {
    const res = await fetch('/api/rsvps', {
      ...
      body: JSON.stringify({ slug, name: nameVal, email: emailVal, guests: guestsVal, message: messageVal, attending }),
    });
...
if (!CSS.supports('animation-timeline: view()') && window.matchMedia('(prefers-reduced-motion: no-preference)').matches) {
  import('https://esm.sh/gsap@3.12.2').then(({ default: gsap }) => {
    import('https://esm.sh/gsap@3.12.2/ScrollTrigger').then(({ ScrollTrigger }) => {
